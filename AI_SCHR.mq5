//+------------------------------------------------------------------+
//|                                                      AI_SHCR.mq5 |
//|                             Copyright 2018, Shcherbyna Rostyslav |
//| Using SCHR Indicator                                             |
//| Work on Open Prices                                              |
//+------------------------------------------------------------------+
#property copyright "Copyright 2018, Shcherbyna Rostyslav"
#property link      ""
#property version   "1.00"
//+------------------------------------------------------------------+
//| Indicators Versions                                              |
//+------------------------------------------------------------------+
enum ENUM_IND_VERSION
  {
   RSI,
   Empire8,
   Empire_TDF,
...
private:
   ...
   string            m_ind_custom_path;
   int               m_ind_rsi_handle;
   int               m_ind_empire_handle;
   int               m_ind_custom_handle;
   ...
   //Current Ind Version
   ENUM_IND_VERSION  m_current_ind;
...
public:
   ...
   double            CUSTOM_IND(void)     const {return(m_arr_custom[m_ind_last_indx]);}
...
//Get Maximum Availables Volume on broker
   ...
   m_ind_custom_path=is.ind_Custom_Path;
   m_current_ind=is._Ind_Version;
      ...
      if(signal==DBL_BUY)
        {
         signal=BUY;
         lot=lot*2;
         m_PartiallySendOrder(lot,signal,"2B ");
         return;
        }
      ...
      if(signal==DBL_SELL)
        {
         signal=SELL;
         lot=lot*2;
         m_PartiallySendOrder(lot,signal,"2S ");
         return;
        }
   ...
   switch(m_current_ind)
     {
      ...
      case RSI:
         ...
         if(m_ind_rsi_handle<0) return(false);
         else return(true);
         break;

      case Empire_TDF:
         //Connect TDF
         m_ind_custom_handle=iCustom(_Symbol,0,m_ind_custom_path);
         break;
      default: m_ind_custom_handle=iCustom(_Symbol,0,m_ind_custom_path);
      break;
     }

// is Indicators Created?
   if(m_ind_custom_handle<0)
      return(false);
   else
      return(true);
...
//Save Last Ind_Value_Index
   ...
   switch(m_current_ind)
     {
      case  Empire8:
         if((BarsCalculated(m_ind_empire_handle)<0)) return(false);
      ...
      case RSI:
         if((BarsCalculated(m_ind_rsi_handle)<0)) return(false);
         if(CopyBuffer(m_ind_rsi_handle,0,0,CopyCount,m_arr_rsi)<0) return(false);
         break;

      case Empire_TDF:
         if((BarsCalculated(m_ind_custom_handle)<0)) return(false);
         if(CopyBuffer(m_ind_custom_handle,4,0,CopyCount,m_arr_custom)<0) return(false);
         break;
      default: if(CopyBuffer(m_ind_custom_handle,4,0,CopyCount,m_arr_custom)<0) return(false);
      break;
     }
   ...
   switch(m_current_tr)
     {
      ...
      case  TDF:m_current_signal=m_TR_TDF();                                return;break;
...
//| TR TDF                                                           |
//+------------------------------------------------------------------+
ENUM_TR_SIGNAL AI_Empire::m_TR_TDF(void)
  {
//Last TDF
   double tdf=m_arr_custom[m_ind_last_indx];

//Check if TDF = 0,1 - NO_TRADE!
   if(tdf==0) return(NOTRADE);
...
//Check TR
//BUY
   if(tdf==BUY) m_current_signal=BUY;

//SELL
   else    if(tdf==SELL) m_current_signal=SELL;
...
input ENUM_IND_VERSION _IND_Version=Empire_TDF;                   // Ind Version
...
string input         _Custom_Path="RIndicators//EmpireTDF";       // Ind Custom Path
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   is.ind_Custom_Path=_Custom_Path;
   is._Ind_Version=_IND_Version;
